...
`include "common_cells/registers.svh"
  ...
  always_comb begin : operand_req_proc
    ...
    if (&(result_valid | ~pending_results) && !result_tag.reduction) begin
      ...
      if (result_tag.narrowing) begin
        ...
        vreg_wbe = vrf_be_t'((1 << ((spatz_req.vl + 7) >> 3)) - 1);
      end
    end
  ...
  // Counter used in VFCMP align result
        ...
        unique case (spatz_req.vtype.vsew)
          EW_8: begin
            for (int i = 0; i < VRFWordWidth/8; i++)
                vreg_wdata[i+(VRFWordWidth/8*cnt_q)] = result[i*8];
          end
          EW_16: begin
            for (int i = 0; i < VRFWordWidth/16; i++)
                vreg_wdata[i+(VRFWordWidth/16*cnt_q)] = result[i*16];     
          end
          EW_32: begin
            for (int i = 0; i < VRFWordWidth/32; i++)
                vreg_wdata[i+(VRFWordWidth/32*cnt_q)] = result[i*32];                
          end
          EW_64: begin
            for (int i = 0; i < VRFWordWidth/64; i++)
                vreg_wdata[i+(VRFWordWidth/64*cnt_q)] = result[i*64];                     
          end 
          default:; 
        endcase
    end
